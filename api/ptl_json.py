...
def json_read(filename):
    try:
        ...
        sys.stderr.write("Coudln't open json file")
        sys.exit()
    else:
        return json_parsed

def json_value(filename, tag, tag2=None):
...
def update_status(filename, status, open_closed):
    json_parsed = json_read(filename)
    json_parsed["open"] = open_closed
    json_parsed["status"] = status
    json_parsed["lastchange"] = int(time.time())
    try:
        json_file = open(filename, "w")
        json_file.write(json.dumps(json_parsed, sort_keys=True, indent=4, separators=(",",": ")))
    except IOError:
        sys.stderr.write("Coudln't write json file")
        sys.exit()
    else:
	return True
