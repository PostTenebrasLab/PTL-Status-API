...
def open_closed_img():
	...
	return static_file('img/open_small.png', root=ROOT_FOLDER, mimetype='image/png')
    else:
	return static_file('img/closed_small.png', root=ROOT_FOLDER, mimetype='image/png')
...
def open_closed_img():
    ...
    response.set_header('Expires', '0')
    if status_json.json_value(JSON_FILENAME, "state", "open") == "True":
	return static_file('img/open_logo.png', root=ROOT_FOLDER, mimetype='image/png')
    else:
	return static_file('img/closed_logo.png', root=ROOT_FOLDER, mimetype='image/png')
...
# Return normal PTL logo when closed and with open logo (green "lab" text) when open
@get('/img/logo.png')
def open_closed_img():
    response.set_header('Cache-Control', 'no-cache, no-store, must-revalidate')
    response.set_header('max age', '0')
	...
	return static_file('img/normal_logo.png', root=ROOT_FOLDER, mimetype='image/png')
